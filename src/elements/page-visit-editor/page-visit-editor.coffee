...
Polymer {
  ...
  addAsFavoriteExamination: (e)->
    if ((@comboBoxExaminationInputValue is '') or (@comboBoxExaminationInputValue is null))
      @domHost.showToast 'Type/Add a Test first!'
      return
    @addExaminationAsFavorite @comboBoxExaminationInputValue
  ...
  _duplicatieNoteCheck: (noteIdentifier)->
    ...
    if list.length > 0
      for item in list
        if item.data.name is noteIdentifier
          @domHost.showToast 'Already exist on your list!'
          return true
        else
          continue
  ...
  addAsFavoriteNote: (e)->
    if ((@doctorNotesMessage is '') or (@doctorNotesMessage is null))
      @domHost.showToast 'Type/Add a Note first!'
      return
    @addNoteAsFavorite @doctorNotesMessage
  ...
  _loadSettings: ()->
    ...
    if settings
      ...
      if (settings.flags.hasOwnProperty 'hideMedicineManufacturerName') is false
        settings.flags.hideMedicineManufacturerName = false

      if (settings.flags.hasOwnProperty 'hideMedicineGenericName') is false
        settings.flags.hideMedicineGenericName = false

      @settings = settings


  getHideMedicineGenericName: ()->
    if @settings.flags.hasOwnProperty 'hideMedicineGenericName'
      return settings.flags.hideMedicineGenericName
