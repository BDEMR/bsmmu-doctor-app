...
Polymer {
  ...
  $makeDateFromTimeStamp: (createdDate)->
    itemCreated = new Date createdDate
    
    year = itemCreated.getFullYear()
    month = (itemCreated.getMonth()+1).toString()
    date = itemCreated.getDate().toString()
  ...
  addCurrentOperation: (e) ->
    # console.log 'type ', typeof @currentOperationValue
    # console.log @currentOperationValue
    console.log 'current operation list ', @currentOperationList

    trimmedVal = @currentOperationValue.trim()
    alreadyAdded = @_duplicateCurrentOperationNameCheck trimmedVal
    if trimmedVal is '' 
      @domHost.showToast 'Type/Add an Operation first!'
      return
    else if alreadyAdded
      # check if added on the same day
      result = @_duplicateCurrentOperationDateCheck trimmedVal
      if result.duplicate
        @domHost.showModalDialog "Already added on #{result.date}!"
        return
      else
        # ask permission whether to add it again
        @domHost.showModalPrompt "Previously added on #{result.date}, Add it again today?", (answer)=>
          if answer is true
            @_finallyAddOperation trimmedVal
          else
            return
    else
      @_finallyAddOperation trimmedVal
  ...
  _duplicateCurrentOperationNameCheck: (newOp)->
    for operation in @currentOperationList
      return true if operation.operation is newOp


  _duplicateCurrentOperationDateCheck: (newOp)->
    todaysDate = @$getTodayDate()
    result = 
      duplicate: false
      date: ''
    for operation in @currentOperationList
      if operation.operation is newOp
        creationDate = @$makeDateFromTimeStamp operation.createdDatetimeStamp
        if todaysDate is creationDate
          result = {duplicate:true, date:todaysDate}
          return result
        else
          result = {duplicate:false, date:creationDate}
  ...
  _finallyAddOperation: (trimmedVal)->
    ...
    app.db.upsert "current-operation", operation, ({serial})-> serial is operation.serial
  ...
  addPreviousOperation: (e) ->
    ...
    trimmedVal = @previousOperationValue.trim()
    alreadyAdded = @_duplicatePreviousOperationNameCheck trimmedVal
    if trimmedVal is ''
      @domHost.showToast 'Type/Add an Opertion first!'
      return
    else if alreadyAdded
      # check if added on the same day
      result = @_duplicatePreviousOperationDateCheck trimmedVal
      if result.duplicate
        @domHost.showModalDialog "Already added on #{result.date}!"
        return
      else
        ...
        @domHost.showModalPrompt "Previously added on #{result.date}, Add it again today?", (answer)=>
          if answer is true
            @_finallyAddPreviousOperation trimmedVal
          else
            return
    else
      @_finallyAddPreviousOperation trimmedVal
  ...
  _duplicatePreviousOperationNameCheck: (prevOp)->
    for operation in @previousOperationList
      return true if operation.operation is prevOp


  _duplicatePreviousOperationDateCheck: (prevOp)->
    todaysDate = @$getTodayDate()
    result = 
      duplicate: false
      date: ''
    for operation in @previousOperationList
      if operation.operation is prevOp
        creationDate = @$makeDateFromTimeStamp operation.createdDatetimeStamp
        if todaysDate is creationDate
          result = {duplicate:true, date:todaysDate}
          return result
  ...
  _finallyAddPreviousOperation: (trimmedValue)->
    operation =
      serial: @generateSerialForOperation()
      createdDatetimeStamp: lib.datetime.now()
      lastModifiedDatetimeStamp: lib.datetime.now()
      ...
      doctorName: @$getFullName @user.name
      doctorSpeciality: @getDoctorSpeciality()
      operation: trimmedValue
  
    @push "previousOperationList", operation
    array = @previousOperationList
    @previousOperationList = []
    @previousOperationList = array
    app.db.upsert "previous-operation", operation, ({serial})-> serial is operation.serial
  ...
  diagnosisModalConfirm: (e) ->
    console.log 'diagnosis list ', @confirmedDiagnosisList 
    if e.detail.confirmed
      trimmedVal = @confirmedDiagnosisModalValue.trim()
      alreadyAdded = @_duplicateNewDiagnosisNameCheck trimmedVal
      if trimmedVal is ''
        @domHost.showToast 'Add/Type a Diagnosis!'
        return
      else if alreadyAdded
        # check if added on the same day
        result = @_duplicateNewDiagnosisDateCheck trimmedVal
        if result.duplicate
          @domHost.showModalDialog "Already added on #{result.date}!"
          return
        else
          ...
          @domHost.showModalPrompt "Previously added on #{result.date}, Add it again today?", (answer)=>
            if answer is true
              @_finallyAddDiagnosis trimmedVal
            else
              return
      else
        @_finallyAddDiagnosis trimmedVal
  ...
  _duplicateNewDiagnosisNameCheck: (newDiagnosis)->
    for diagnosis in @confirmedDiagnosisList
      return true if diagnosis.diagnosis is newDiagnosis

  _duplicateNewDiagnosisDateCheck: (newDiagnosis)->
    todaysDate = @$getTodayDate()
    result = 
      duplicate: false
      date: ''
    ...
    for diagnosis in @confirmedDiagnosisList
      if diagnosis.diagnosis is newDiagnosis
        creationDate = @$makeDateFromTimeStamp diagnosis.createdDatetimeStamp
        if todaysDate is creationDate
          result = {duplicate:true, date:todaysDate}
          return result
        else
  ...
  _finallyAddDiagnosis: (trimmedValue)->
    diagnosis = {
      ...
      doctorName: @$getFullName @user.name
      doctorSpeciality: @getDoctorSpeciality()
      diagnosis: trimmedValue
    }
    @push "confirmedDiagnosisList", diagnosis
    array = @confirmedDiagnosisList
    @confirmedDiagnosisList = []
    @confirmedDiagnosisList = array
    app.db.upsert "visit-diagnosis", diagnosis, ({serial})-> serial is diagnosis.serial
  ...
  currentDiagnosisModalConfirm: (e) ->
    if e.detail.confirmed
      trimmedVal = @currentDiagnosisModalValue.trim()
      alreadyAdded = @_duplicateCurrentDiagnosisNameCheck trimmedVal
      if trimmedVal is ''
        @domHost.showToast 'Type/Add a Diagnosis first!'
        return
      else if alreadyAdded
        # check if added on the same day
        result = @_duplicateCurrentDiagnosisDateCheck trimmedVal
        if result.duplicate
          @domHost.showModalDialog "Already added on #{result.date}!"
          return
        else
          ...
          @domHost.showModalPrompt "Previously added on #{result.date}, Add it again today?", (answer)=>
            if answer is true
              @_finallyAddCurrentDiagnosis trimmedVal
            else
              return
      else
        @_finallyAddCurrentDiagnosis trimmedVal
  ...
  _duplicateCurrentDiagnosisNameCheck: (newDiagnosis)->
    for diagnosis in @currentDiagnosisList
      return true if diagnosis.diagnosis is newDiagnosis


  _duplicateCurrentDiagnosisDateCheck: (newDiagnosis)->
    todaysDate = @$getTodayDate()
    result = 
      duplicate: false
      date: ''
  ...
  _finallyAddCurrentDiagnosis: (trimmedValue)->
    diagnosis = {
      serial: @generateSerialForDiagnosis()
      createdDatetimeStamp: lib.datetime.now()
      lastModifiedDatetimeStamp: lib.datetime.now()
    ...
    app.db.upsert "visit-current-diagnosis", diagnosis, ({serial})-> serial is diagnosis.serial
  ...
  addCommentButtonClicked: (e) ->
    ...
    unless @doctorCommentMessage.data.message is null
      trimmedVal = @doctorCommentMessage.data.message.trim()
      if trimmedVal is ''
        @domHost.showToast 'Add a Note first!'
        return
      else
