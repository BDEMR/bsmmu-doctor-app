...
Polymer {
  ...
  _hideCard: (type) ->
    ...
    list = @settings.visit.cards

    console.log list.indexOf type

    if (list.indexOf type is -1)
      return false

    item = list[(list.indexOf type)]
    console.log({item})
  ...
  _loadUser:()->
    ...
    if list.length is 1
      ...
      return @domHost.showModalDialog 'Invalid User!'
  ...
  _resetMedicineForm: ()->
    @medicine =
      ...
      status: 'continue'
      flags:
        hideMedicineManufacturerName: false
        hideMedicineGenericName: false
  ...
  cancelButtonClicked: (e)->
    @_resetMedicineForm()
    @_disablePrescriptionFormField false
  ...
  _getMedicineFormSelectionIndex: (cbName)->
    for form, index in @medicineFormList
      return index unless cbName.indexOf(form) is -1
    return null;


  brandNameAutocompleteSelected: (e)->
    brandName = e.detail.value
    compositeBrandName = e.detail.text
    ...
    if @matchingMedicineList.length isnt 0
      ...
      # find which form is in the composite brand name
      match = @_getMedicineFormSelectionIndex compositeBrandName
      console.log 'found match at ', match
      if match is null
        match = 0
      else
        @_disablePrescriptionFormField true

      @set 'medicineFormSelectedIndex', null
      # HACK - to fix setting selected index to other paper-menu element
      lib.util.delay 5, ()=>
        @set 'medicineFormSelectedIndex', match
      # HACK End
  ...
  _disablePrescriptionFormField: (disable)->
  ...
  saveMedicineButtonClicked: (e)->
    ...
    @_checkValidity inputsToValidate, (status)=>
      if status
        ...
        ## flags
        if @settings
          medicine.data.flags.hideMedicineManufacturerName = @settings.flags.hideMedicineManufacturerName or false
          medicine.data.flags.hideMedicineGenericName = @settings.flags.hideMedicineGenericName or false
  ...
  _duplicateMedicineGenericNameCheck: (genericName)->
    for medicine in @matchingPrescribedMedicineList
      return true if medicine.data.genericName is genericName
  ...
  _duplicateIdentifiedSymptom: (newSymptom)->
    for symptomName in @addedIdentifiedSymptomsList
      if symptomName.name is newSymptom
        return true
    

  saveIdentifiedSymptoms: ()->
    console.log @addedIdentifiedSymptomsList
    # console.log @identifiedSymptomsObject
  ...
  # Favorite symptoms - start ---------------------->

  loadFavoriteSymptomList: (userIdentifier, cbfn)->
    list = app.db.find 'user-favorite-symptom', ({createdByUserSerial})-> userIdentifier is createdByUserSerial 
    list.sort (left, right)->
      return -1 if left.data.name > right.data.name
      return 1 if left.data.name < right.data.name
      return 0

    @favoriteSymptomList = list
    console.log 'favoriteSymptomList', @favoriteSymptomList

    cbfn() 

  _duplicatieSymptomCheck: (symptomIdentifier)->
    userIdentifier = @user.serial
    list = app.db.find 'user-favorite-symptom', ({createdByUserSerial})-> userIdentifier is createdByUserSerial
    if list.length > 0
      for item in list
        if item.data.name is symptomIdentifier
          @domHost.showToast 'Already exist on your list!'
          return true
        else
          continue
      return false
  ...
  showFavoriteSymptomDialog: ()->
    @loadFavoriteSymptomList @user.serial, =>
      @$$('#dialogFavoriteSymptom').toggle()


  addSymptomFromFavorite: ()->
    console.log @favoriteSymptomList
    alreadyAddedItems = []
    for item in @favoriteSymptomList
      if item.isSelected
        if @_duplicateIdentifiedSymptom item.data.name
          alreadyAddedItems.push item.data.name
        else
          @push 'addedIdentifiedSymptomsList', item.data

    unless alreadyAddedItems.length is 0
      itemsTogether = alreadyAddedItems.reduce (result, value)->
        return result += (', '+value)
      
      @domHost.showToast ('These Symptoms were already added : '.concat itemsTogether)
      console.log 'already added items ', alreadyAddedItems
    
    @$$('#dialogFavoriteSymptom').close()


  deleteSelectedFavoriteSymptom: (e)->
    index = e.model.index

    item = @favoriteSymptomList[index]
    @splice 'favoriteSymptomList', index, 1

    app.db.remove 'user-favorite-symptom', item._id
    app.db.insert 'user-favorite-symptom--deleted', { 
      serial: item.serial
      lastModifiedDatetimeStamp: lib.datetime.now()
      clientCollectionName: 'user-favorite-symptom--deleted'
    }
  ...
  addSymptomAsFavorite: (data)->
    ...
    if @_duplicatieSymptomCheck data.name
      ...
      object =
        serial: @generateSerialForFavoriteSymptom()
        createdByUserSerial: @user.serial
        lastModifiedDatetimeStamp: lib.datetime.now()
        organizationId: @currentOrganization.idOnServer
  ...
  addAsFavoriteSymptom: (e)->
    tempSymptomObj = {}
    if typeof @comboBoxSymptomsInputValue is 'object'
      # deal with object symptom
      tempSymptomObj = @comboBoxSymptomsInputValue
    else
      # custom symptom name string
      trimmedVal = @comboBoxSymptomsInputValue.trim() unless @comboBoxSymptomsInputValue is null 
      unless trimmedVal is ''
        tempSymptomObj = {name:trimmedVal}
      else
        @domHost.showToast 'Type/Add a Symptom first!'
        return
      
    @addSymptomAsFavorite tempSymptomObj

  
  # Favorite symptoms - end ---------------------->

  # Favorite examination - start ---------------------->

  loadFavoriteExaminationList: (userIdentifier, cbfn)->
    list = app.db.find 'user-favorite-examination', ({createdByUserSerial})-> userIdentifier is createdByUserSerial 
    list.sort (left, right)->
      return -1 if left.data.name > right.data.name
      return 1 if left.data.name < right.data.name
      return 0

    @favoriteExaminationList = list
    console.log 'favoriteExaminationList', @favoriteExaminationList

    cbfn() 

  _duplicatieExaminationCheck: (examinationIdentifier)->
    userIdentifier = @user.serial
    list = app.db.find 'user-favorite-examination', ({createdByUserSerial})-> userIdentifier is createdByUserSerial
    if list.length > 0
      for item in list
        if item.data.name is examinationIdentifier
          @domHost.showToast 'Already exist on your list!'
          return true
        else
          continue
  ...
  showFavoriteExaminationDialog: ()->
    @loadFavoriteExaminationList @user.serial, =>
      @$$('#dialogFavoriteExamination').toggle()


  addExaminationFromFavorite: ()->
    # console.log @favoriteExaminationList
    alreadyAddedItems = []
    for item in @favoriteExaminationList
      if item.isSelected
        if @_duplicateAddedExamination item.data.name
          alreadyAddedItems.push item.data.name
        else
          @push 'addedExaminationList', item.data

    unless alreadyAddedItems.length is 0
      itemsTogether = alreadyAddedItems.reduce (result, value)->
        return result += (', '+value)
      
      @domHost.showToast ('These Examinations were already added : '.concat itemsTogether)
      console.log 'already added items ', alreadyAddedItems
 
    @$$('#dialogFavoriteExamination').close()


  deleteSelectedFavoriteExamination: (e)->
    index = e.model.index

    item = @favoriteExaminationList[index]
    @splice 'favoriteExaminationList', index, 1

    app.db.remove 'user-favorite-examination', item._id
    app.db.insert 'user-favorite-examination--deleted', { 
      serial: item.serial
      lastModifiedDatetimeStamp: lib.datetime.now()
      clientCollectionName: 'favorite-advised-test--deleted'
    }

    @domHost.showToast 'Test removed from favorite list'
  

  addExaminationAsFavorite: (data)->
    console.log data
    if @_duplicatieExaminationCheck data.name
      return
    else
      object =
        serial: @generateSerialForFavoriteExamination()
        createdByUserSerial: @user.serial
        lastModifiedDatetimeStamp: lib.datetime.now()
        organizationId: @currentOrganization.idOnServer
        clientCollectionName: 'user-favorite-examination'
        isSelected: false
        data: data

      app.db.insert 'user-favorite-examination', object
      @domHost.showToast 'Added to favorite list'

  addAsFavoriteExamination: (e)->
    tempExaminationObj = {}
    if typeof @comboBoxExaminationInputValue is 'object'
      # deal with object examination
      tempExaminationObj = @comboBoxExaminationInputValue
    else
      # custom examination name string
      trimmedVal = @comboBoxExaminationInputValue.trim() unless @comboBoxExaminationInputValue is null 
      unless trimmedVal is ''
        tempExaminationObj = {name:trimmedVal}
      else
        @domHost.showToast 'Type/Add an Examination first!'
        return

    @addExaminationAsFavorite tempExaminationObj
  
  
  # Favorite examination - end ---------------------->
  ...
  _duplicateExaminationMember: (membersList, newMember)->
    for examination in membersList
      if examination.value is newMember
        return true
  ...
  _duplicateAddedExamination: (newExamination)->
    for examinationName in @addedExaminationList
      if examinationName.name is newExamination
        return true
  ...
  _makeCustomInvestigationObject : (cbfn)->
    @customInvestigationObject = {
      ...
      # visitSerial: @visit.serial
      # patientSerial: @patient.serial
      clientCollectionName: 'custom-investigation-list'
      organizationId: @currentOrganization.idOnServer
      data:
        name: ''
        investigationList: [
  ...
  saveUserAddedInstituion: ()->
    userAddedInsitution = {
      ...
      createdByUserSerial: @user.serial
      organizationId: @currentOrganization.idOnServer
      clientCollectionName: 'user-added-institution-list'
      data:
  ...
  addInvestigation: ()->
    console.log 'added investigation list ', @addedInvestigationList
    console.log 'selected type ', typeof @comboBoxInvestigationInputValue
    console.log 'selected obj ', @comboBoxInvestigationInputValue
  ...
  _saveCustomInvestigation: ()->
    ...
    @_loadInvestigationList @currentOrganization.idOnServer
  ...
  loadFavoriteInvestigationList: (userIdentifier, cbfn)->
    ...
    console.log 'favoriteInvestigaitonList', @favoriteInvestigaitonList
  ...
  _duplicatieInvestigationCheck: (investigationIdentifier)->
    userIdentifier = @user.serial
    list = app.db.find 'favorite-advised-test', ({createdByUserSerial})-> userIdentifier is createdByUserSerial
    console.log 'investigatoin list', list
    if list.length > 0
      for item in list
        if item.data.name is investigationIdentifier
          @domHost.showToast 'Already exist!'
          return true
        else
          continue
      return false
  ...
  deleteSelectedFavoriteInvestigation: (e)->
    ...
    item = @favoriteInvestigaitonList[index]
    @splice 'favoriteInvestigaitonList', index, 1

    app.db.remove 'favorite-advised-test', item._id
    app.db.insert 'favorite-advised-test--deleted', { 
      serial: item.serial
      lastModifiedDatetimeStamp: lib.datetime.now()
      clientCollectionName: 'favorite-advised-test--deleted'
      }
  ...
  addInvestigationAsFavorite: (data)->
    if @_duplicatieInvestigationCheck data.name
      ...
      object =
        ...
        lastModifiedDatetimeStamp: lib.datetime.now()
        organizationId: @currentOrganization.idOnServer
        clientCollectionName: 'favorite-advised-test'
        isSelected: false
        data: data
      console.log 'addInvestigationAsFavorite', object
  ...
  _addNextVisitPressed: ()->
    console.log 'datetime now ', lib.datetime.now()
  ...
  _addNotePressed: ()->
    unless @doctorNotesMessage is ''
      trimmedVal = @doctorNotesMessage.trim()
      if trimmedVal is ''
        @domHost.showToast 'Add/Type a Note first!'
        return
      else if @_duplicateAddedNoteCheck trimmedVal
        @domHost.showToast 'Exact note is already added!'
        return
      else
        @push 'doctorNotes.data.messageList', trimmedVal
  ...
  _duplicateAddedNoteCheck: (newNote)->
    for note in @doctorNotes.data.messageList
      return true if note is newNote


  # Favorite notes - start ---------------------->

  loadFavoriteNoteList: (userIdentifier, cbfn)->
    list = app.db.find 'user-favorite-note', ({createdByUserSerial})-> userIdentifier is createdByUserSerial 
    list.sort (left, right)->
      return -1 if left.data.name > right.data.name
      return 1 if left.data.name < right.data.name
      return 0

    @favoriteNoteList = list
    console.log 'favoriteNoteList', @favoriteNoteList

    cbfn() 

  _duplicatieNoteCheck: (noteIdentifier)->
    userIdentifier = @user.serial
    list = app.db.find 'user-favorite-note', ({createdByUserSerial})-> userIdentifier is createdByUserSerial
    
    if list.length > 0
      for item in list
        if item.data is noteIdentifier
          @domHost.showToast 'Already exist on your list!'
          return true
        else
          continue
  ...
  showFavoriteNoteDialog: ()->
    @loadFavoriteNoteList @user.serial, =>
      @$$('#dialogFavoriteNote').toggle()


  addNoteFromFavorite: ()->
    # console.log @favoriteNoteList
    alreadyAddedItems = []
    for item in @favoriteNoteList
      if item.isSelected
        if @_duplicateAddedNoteCheck item.data
          alreadyAddedItems.push item.data
        else
          @push 'doctorNotes.data.messageList', item.data

    unless alreadyAddedItems.length is 0
      itemsTogether = alreadyAddedItems.reduce (result, value)->
        return result += (', '+value)
      
      @domHost.showToast ('These Notes were already added : '.concat itemsTogether)   
      console.log 'already added items ', alreadyAddedItems

    @$$('#dialogFavoriteNote').close()


  deleteSelectedFavoriteNote: (e)->
    index = e.model.index

    item = @favoriteNoteList[index]
    @splice 'favoriteNoteList', index, 1

    app.db.remove 'user-favorite-note', item._id
    app.db.insert 'user-favorite-note--deleted', { 
      serial: item.serial
      lastModifiedDatetimeStamp: lib.datetime.now()
      clientCollectionName: 'favorite-note--deleted'
    }

    @domHost.showToast 'Test removed from favorite list'
  

  addNoteAsFavorite: (data)->
    console.log data
    if @_duplicatieNoteCheck data
      return
    else
      object =
        serial: @generateSerialForFavoriteNote()
        createdByUserSerial: @user.serial
        lastModifiedDatetimeStamp: lib.datetime.now()
        organizationId: @currentOrganization.idOnServer
        clientCollectionName: 'user-favorite-note'
        isSelected: false
        data: data

      app.db.insert 'user-favorite-note', object
      @domHost.showToast 'Added to favorite list'

  addAsFavoriteNote: (e)->
    unless @doctorNotesMessage is null
      trimmedNote = @doctorNotesMessage.trim()
      if trimmedNote is ''
        @domHost.showToast 'Add/Type a Note first!'
        return
      else
        @addNoteAsFavorite trimmedNote
  ...
  _saveVisit: ()->
    ...
    if params['visit'] is 'new'
      ...
      @_callAuthorizeRecordApi @visit.serial, @visit.serial, 'Visit Record', 'bdemr--doctor-visit', @currentOrganization.idOnServer, =>
  ...
  _loadSettings: ()->
    if !(@settings.flags.hasOwnProperty 'hideMedicineManufacturerName')
      @settings.flags.hideMedicineManufacturerName = false

    if !(@settings.flags.hasOwnProperty 'hideMedicineGenericName')
      @settings.flags.hideMedicineGenericName = false
  ...
  _loadVariousWallets: ->
    ...
    @async => @_loadPatientOrganizationWallet @currentOrganization.idOnServer, @patient.idOnServer, (patientOrganizationWallet)=>
  ...
  _callAuthorizeRecordApi: (visitSerial, recordSerial, recordType, serverDb, organizationId, cbfn)->
    ...
    data = { 
      ...
      patientSerial: @patient.serial
      recordSubType: @visit.recordTitle
    }
  ...
  _preloadData: ->
    ...
    @async => @_loadInsulinList params['patient']
    @async => @_listFavoriteMedicine user.serial
    @async => @_listCurrentMedications params['patient']
    # Preloaded Data - Symptoms
    @async => @_loadSymptomsListFromSystem user.serial
    # Preloaded Data - Examination
    @async => @_loadExaminationList user.serial
    # Preloaded Data - Test Advised
    @async => @_loadInvestigationList organization.serial
    @async => @_loadUserAddedInstitutionList user.serial
  ...
  navigatedIn: ->
    ...
    # Load Patient Stay
    @_loadPatientStayObject @currentOrganization.idOnServer, ()=>
    ...
    if params['visit'] is 'new'
      @_makeNewVisit =>
        @async => @_makeNewPrescription()
        @async => @_makeNewIdentifiedSymptomsObject()
        @async => @_makeNewExaminationObject()

        @async => @_makeBloodPressure()
        @async => @_makePulseRate()
        @async => @_makeBmi()
        @async => @_makeRespiratoryRate()
        @async => @_makeOxygenSaturation()
        @async => @_makeTemperature()

        @async => @_makeNewTestAdvisedObject()
        @async => @_makeNewDiagnosis()
        @async => @_makeNewNote()
        @async => @_makeNewNextVisit()
        @async => @invoice = {}
        @async => @_loadOrganizationsIBelongTo()
        @async => @_makeNewPatientStay()

    else
      @async => @_loadVisit params['visit'], =>
        ...
        if @visit.hasOwnProperty('prescriptionSerial')
          if @visit.prescriptionSerial
            @async => @_loadPrescription @visit.prescriptionSerial
          else
            @_makeNewPrescription()
        else
    ...
    title.innerText =  patientName + "_" + @patient.serial

    # load Wallets
    @async => @_loadVariousWallets => null
  ...
  historyAndPhysicalRecordRemove: (e)->
    console.log @historyAndPhysicalRecord
    @domHost.showModalPrompt 'Are you sure?', (answer)=>
      if answer
        app.db.remove 'history-and-physical-record', @historyAndPhysicalRecord._id
        @set 'visit.historyAndPhysicalRecordSerial', null
        @_saveVisit()
        @historyAndPhysicalRecord = null
  ...
  _makeNewDiagnosis: ()->
    @diagnosis =
      ...
      patientSerial: @patient.serial
      patientName: @$getFullName @patient.name
      patientAge: @_computeAge @patient.dateOfBirth
      patientGender: @patient.gender
      doctorName: @visit.doctorName
      doctorSpeciality: @visit.doctorSpeciality
      organizationId: @currentOrganization.idOnServer
      patientStayDetails: null
      data:
        diagnosisList: []
  
  getThisPatientStayDetails:()->
    patientStayDetails = null
      ...
      for seat in seatList
        if seat.patientSerial is @patient.serial
          patientStayDetails =
            ...
            unit: seat.unit
            ward: seat.ward
            admittedDatetimeStamp: seat.patientAdmittedDatetimeStamp
  ...
  _saveDiagnosis: ()->
    ...
    @diagnosis.patientStayDetails = @getThisPatientStayDetails() 
  ...
  # Patient Stay START
  togglePatientStayDetails: ()->
    @showPatientStayDetails = !@showPatientStayDetails
  ...
  _gotoPatientQuickPreviewPage: ->
    @domHost.navigateToPage  '#/patient-quick-info/patient:' + @patient.serial
  ...
  _generateSymptomCategory:(symptomList)->
    ...
    console.log @symptomsCategoryList

  symptomCategorySelected:(e)->
    index = e.detail.selected
    unless index is 0
      category = @symptomsCategoryList[index]
      filteredList = @symptomsDataList.filter (item)=> item.category is category
