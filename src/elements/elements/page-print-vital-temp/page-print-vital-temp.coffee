...
Polymer {
  ...
  _listVitalTemperature: (patientIdentifier) ->
    ...
    if params['startDate'] isnt 'null' and params['startDate'] isnt 'undefined' and params['endDate'] isnt 'null' and params['endDate'] isnt 'undefined'
      ...
      filterdBPList = (item for item in @matchingVitalTemperatureList when (startDate.getTime() <= (new Date item.lastModifiedDatetimeStamp).getTime() <= endDate.getTime()))
      @matchingVitalTemperatureList = filterdBPList
